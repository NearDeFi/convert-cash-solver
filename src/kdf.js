...
// hacky solution
async function uncompressedHexPointToNearImplicitEcdsa(uncompressedHexPoint) {
    const implicitPublicKeyEcdsa =
        'secp256k1:' +
        baseEncode(Buffer.from(uncompressedHexPoint.substring(2), 'hex'));
    // get an implicit accountId from an ed25519 keyPair using the sha256 of the secp256k1 point as entropy
    const sha256HashOutput = await crypto.subtle.digest(
        'SHA-256',
        Buffer.from(uncompressedHexPoint, 'hex'),
    );
    const { publicKey, secretKey: implicitAccountSecretKeyEcdsa } =
        generateSeedPhrase(Buffer.from(sha256HashOutput));

    // DEBUG
    // console.log(secretKey);

    const implicitAccountIdEcdsa = Buffer.from(
        baseDecode(publicKey.split(':')[1]),
    ).toString('hex');
    ...
    return {
        implicitAccountIdEcdsa,
        implicitPublicKeyEcdsa,
        implicitAccountSecretKeyEcdsa,
    };
}

async function uncompressedHexPointToNearImplicitEddsa(uncompressedHexPoint) {
    const publicKey = PublicKey.from(uncompressedHexPoint);

    const implicitAccountIdEddsa = Buffer.from(
        baseDecode(publicKey.split(':')[1]),
    ).toString('hex');

    return {
        implicitAccountIdEddsa,
        implicitPublicKeyEddsa: publicKey.toString(),
    };
}
    ...
    switch (chain) {
        ...
        case 'nearEcdsa':
            const {
                ...
                implicitPublicKeyEcdsa,
                implicitAccountSecretKeyEcdsa,
            } = await uncompressedHexPointToNearImplicitEcdsa(childPublicKey);
            address = implicitAccountIdEcdsa;
            nearSecpPublicKey = implicitPublicKeyEcdsa;
            nearImplicitSecretKey = implicitAccountSecretKeyEcdsa;
            break;
        case 'nearEddsa':
            const { implicitAccountIdEddsa, implicitPublicKeyEddsa } =
                await uncompressedHexPointToNearImplicitEddsa(childPublicKey);
            address = implicitAccountIdEddsa;
            publicKey = implicitPublicKeyEddsa;
            break;
    }
